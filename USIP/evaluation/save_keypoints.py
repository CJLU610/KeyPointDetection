...
root = '/home/datassd/modelnet40-test_rotated_numpy/'
output_folder = '/home/datassd/modelnet40/'
gpu_id = 0
is_ensure_keypoint_num = True
desired_keypoint_num = 2
NMS_radius = 0
noise_sigma = 0
downsample_rate = 1
...
# =============== method ================
method = 'tsf'
detector_model_path = '/home/datassd/best.pth'
        ...
        for b in range(anc_pc.size(0)):
            ...
            if is_plot:
                ...
                if anc_type[b].item() == 0:
                    output_folder_new = os.path.join(root, 'original')
                elif anc_type[b].item() == 1:
                    output_folder_new = os.path.join(root, 'rotated')
                rppt = os.path.join(output_folder_new,'%d.npy' % annn[b].item())


                con = np.load(rppt, encoding="latin1")
                x = con[:, 0]
                y = con[:, 1]
                z = con[:, 2]
                x1 = frame_keypoint_np[:, 0]
                y1 = frame_keypoint_np[:, 1]
                z1 = frame_keypoint_np[:, 2]
                fig = plt.figure()
                ax3d = Axes3D(fig)
            ...
            if 'modelnet' == dataset_type:
                ...
                output_file = os.path.join(output_folder_new, '%d.npy' % anc_idx[b].item())
